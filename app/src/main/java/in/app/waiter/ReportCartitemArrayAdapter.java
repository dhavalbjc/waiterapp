...
import java.util.ArrayList;
	...
	class ReportCartitemArrayAdapter extends ArrayAdapter {
		 
	    private Context mContext;
	    private ArrayList<CartItems> mList;
	    private int index;
	 
	    public ReportCartitemArrayAdapter(Context context, ArrayList<CartItems> list) {
	 ...
	 //       this.index=index;
	    ...
	    public View getView(final int position, View convertView, ViewGroup parent) {
	        ...
	        //price.setText( String.valueOf( mList.get(position).get_item().get_price()) );
	     ...
	     /*   try{
	        ...
	        try{
				...
				if(mList.get(position).get_item().get_flag().equals("I")){
					...
					if(!(Float.parseFloat(mList.get(position).get_item().get_price())>0.0)){
